...
public class Admin_login extends JFrame implements ActionListener{
	...
	Border thickBorder = new LineBorder(new Color(218,227,229), 6);//creating border for center buttons
	...
	public void initialize() {
		...
		//setup the panel no 6	
		p6.setBackground(new Color(218,227,229));
		//setup the panel no 7
	...
	public void actionPerformed(ActionEvent e) {
		//setting the confirm login button
		if(e.getSource()==b1)
		{	
		   ...
		   try 
		   {
			   //connecting to the database
				...
				String query=" Select * from Admins where UserName='"+user+"' and password_='"+pass+"'";
				ResultSet rs=stmt.executeQuery(query);
				//Performing the SQL statement
				if(rs.next())
				{
					this.dispose();
					//moving to next frame on successful login
					admin_frame ef=new admin_frame();
					//driving toward the next frame
				...
				{
					//showing error message in case of incoorect data
		...
		//setting the admin signup button
		if(e.getSource()==b2)
		{
			this.dispose();
			Admin_SignUp as=new Admin_SignUp();
			ImageIcon image = new ImageIcon("E:\\Java\\final project\\train.png");
			as.setIconImage(image.getImage());
			as.setSize(600, 550);
			as.setTitle("E-Ticketing");
			as.setVisible(true);
			as.setResizable(false);
			as.setLocation(200, 100);
